...
// Récupérer les produits de la famille sélectionnée
$familyId = isset($_GET['family']) ? $_GET['family'] : 1; // Remplacez 1 par l'ID de famille par défaut si aucun n'est spécifié
$sqlProduits = "SELECT * FROM produits WHERE id_famille = $familyId";
...
<head>
    ...
    <title>ENG | Catalogue de produits</title>
...
<body>
   ...
   <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <!-- end navbar -->
   <!-- start Catalogue de produits ENG -->
   <section class="container mt-5">
        <h2 class="text-center mb-4">Catalogue de produits</h2>

        <div class="row">
            <!-- Afficher les produits de la famille sélectionnée -->
            <?php
            while ($rowProduits = mysqli_fetch_assoc($resultProduits)) {
                echo '<div class="col-md-4 mb-4">';
                echo '<div class="card">';
                echo '<img src="/eng/assets/img/' . $rowProduits['url_img'] . '" class="card-img-top" alt="' . $rowProduits['nom_produit'] . '">';
                echo '<div class="card-body">';
                echo '<h5 class="card-title">' . $rowProduits['nom_produit'] . '</h5>';
                echo '<p class="card-text">' . $rowProduits['description_produit'] . '</p>';
                echo '<p class="card-text"><strong>Prix:</strong> ' . $rowProduits['prix_produit'] . '</p>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            }
